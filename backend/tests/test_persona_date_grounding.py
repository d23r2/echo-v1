...
def test_system_prompt_includes_current_date_for_every_backend(db_session):
    ...
    prompt, _citations, _nudge_reason, _snippets = persona.build_system_prompt(
...
def test_date_note_defaults_to_real_now_when_not_passed(db_session):
    prompt, _citations, _nudge_reason, _snippets = persona.build_system_prompt(db_session, "hi", turn_count=0)
