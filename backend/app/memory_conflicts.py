...
def find_conflicts(
    db: Session, *, content: str, memory_type: str, tags: list[str]
    ...
    a conflict, and isn't flagged here)."""
    candidate_words = _significant_words(content)
    candidate_tags = {t.lower() for t in tags}
    candidate_content_norm = content.strip().lower()

    conflicts = []
    existing = db.query(models.AtlasEntry).filter(models.AtlasEntry.memory_type == memory_type).all()
...
def find_all_conflicts(db: Session) -> dict[str, list[str]]:
    ...
    entries = db.query(models.AtlasEntry).all()
